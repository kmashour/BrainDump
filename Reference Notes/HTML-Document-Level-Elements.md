...
The `<html>` element should always have exactly two child elements: 

`<head>` and `<body>`. 

The other elements should be descendants of the `<head>` and `<body>`.
...
The `<body>` element is the main container for the content of the page, 
along with any associated markup. 
Generally the `<body>` will have
**`<header>`, 
`<main>`, and
`<footer>`** 
as direct children, but this is not a requirement.
