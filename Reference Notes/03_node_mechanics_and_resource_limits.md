...
```mermaid
graph TD
    A["1. Node Bootstrapping (OS setup, kernel modules, Container Runtime)"] --> B["2. Node Telemetry and Heartbeats (Status checks, Lease API, Eviction)"]
    ...
    D --> E["5. Resource Placement Alignment (CPU, Memory and Device Managers)"]
```
