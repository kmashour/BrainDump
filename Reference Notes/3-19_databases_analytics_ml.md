...
*   **Node Topology:** Consists of a **Leader Node** (receives queries, compiles execution plans, and coordinates compute nodes) and **Compute Nodes** (execute queries and store data).
...
*   **Disaster Recovery:** Supports automated cross-region snapshot replication for disaster recovery.
...
*   **Neptune Streams:** Captures changes to graph data in real-time, exposed via an HTTP REST API to trigger downstream workflows.

### Amazon Timestream (Time Series Database)
*   **Purpose:** Serverless time-series database optimized for IoT telemetry, application metrics, and DevOps monitoring.
*   **Features:** Automatically scales up or down based on load. Keeps recent data in an in-memory store for high-speed writes and transitions older data to a cost-optimized magnetic store based on user-defined retention policies.

### Amazon Keyspaces (Apache Cassandra Compatible)
*   **Purpose:** Serverless, managed Apache Cassandra-compatible database service.
*   **Features:** Uses the Cassandra Query Language (CQL). Scales tables automatically in response to request traffic, offering single-digit millisecond latency, Point-in-Time Recovery (PITR), and multi-region replication.
...
*   **Performance Optimization:**
    ...
    *   **Partitioning:** Organizing S3 paths (e.g., `s3://bucket/year=2026/month=06/`) restricts Athena to scan only files in the queried partitions.
...
*   **Node Types:**
    *   **Master Node:** Manages the cluster, coordinates workloads, and tracks health. Must be long-running.
    *   **Core Node:** Runs tasks and stores data in the Hadoop Distributed File System (HDFS). Must be long-running.
...
*   **Deployment Modes:** Long-running clusters (continuous data processing) or transient clusters (provisioned on-demand, run analysis, and automatically terminate).

### AWS Glue (Managed ETL)
*   **ETL Pipeline:** Serverless Extract, Transform, and Load (ETL) service.
*   **Glue Data Catalog:** Central metadata repository. Glue Crawlers scan S3, RDS, DynamoDB, or JDBC sources to automatically infer schemas and register metadata tables in the catalog. The catalog is shared across Athena, Redshift Spectrum, and EMR.
...
### AWS Lake Formation
*   **Purpose:** An orchestration layer built on top of AWS Glue that simplifies the setup, securing, and management of data lakes in S3.
...
*   **Purpose:** Real-time stream processing framework using SQL, Java, or Scala.
*   **Ingress Options:** Reads streams from Kinesis Data Streams or Amazon MSK.
    *   *Exam Trick:* Flink **cannot** read directly from Kinesis Data Firehose (use Kinesis Data Streams instead, or legacy SQL analytics configurations).
...
*   **Scaling Limit:** You can dynamically add partitions to Kafka topics to scale throughput, but **you cannot decrease partition count**.
...
*   **SPICE Engine:** Super-fast, Parallel, In-memory Calculation Engine. Accelerates dashboard performance by caching data in-memory (requires importing S3, RDS, or Redshift data directly into QuickSight).
*   **User and Security Administration:**
    *   QuickSight manages users and groups independently from IAM. IAM users are reserved for administrative access.
    *   **Column-Level Security (CLS):** Restricts specific columns from being displayed to unauthorized users (Enterprise Edition).
...
*   **Amazon Transcribe:** Converts speech/audio into text using Automatic Speech Recognition (ASR). Supports automatic language identification and automatic redaction of PII (Personally Identifiable Information).
...
*   **Amazon Lex:** Natural Language Understanding (NLU) engine used to build conversational chatbots (same tech powering Amazon Alexa).
*   **Amazon Connect:** Cloud-based contact center. Integrates directly with Lex to analyze customer voice intent and execute backend Lambda functions (e.g., to query databases and schedule appointments).
...
*   **Amazon Kendra:** Document search engine powered by ML. Indexes internal documents (PDF, Word, HTML, FAQs) from multiple data sources, providing natural language search and incremental learning (adjusts results based on user interactions).
...
*   **Amazon Textract:** Automatically extracts text, handwriting, tables, and form fields from scanned PDF/image documents.
...
*   **The ML Lifecycle in SageMaker:**
    1.  **Label:** Prepares and labels training datasets (utilizing Ground Truth).
    2.  **Build:** Notebook environments with pre-loaded frameworks (TensorFlow, PyTorch, Scikit-learn).
    3.  **Train & Tune:** Spins up temporary compute instances to train models, optimizing hyperparameters automatically.
    4.  **Deploy:** Packages models into persistent HTTPS endpoints for real-time predictions.
