---
obsidianUIMode: preview
class: index-note
tier: reference-note
tags:
  - kubernetes/reference-index
  - obsidian/moc
---

# ☸️ Kubernetes Master Reference MOC

**Breadcrumbs:** [[--Index--|🏠 Index]] > **Kubernetes Reference MOC**

> [!IMPORTANT]
> **KubeAstronaut Certification Hub:**
> Access dedicated exam reference maps:
> - ☸️ **[CKA Exam Reference MOC](0-Index%20-%20CKA.md)** (Certified Kubernetes Administrator)
> - 🛡️ **[CKS Exam Reference MOC](0-Index%20-%20CKS.md)** (Certified Kubernetes Security Specialist)
> - 🚀 **[CKAD Exam Reference MOC](0-Index%20-%20CKAD.md)** (Certified Kubernetes Application Developer)

---

## 🏛️ Domain 1: Cluster Architecture & Administration
- 🧠 **[Module 0-2-1: Control Plane Core Architecture](0-2-1_control_plane_and_core_daemons.md)**
- 🏢 **[Module 0-2-2: HA Topologies & Konnectivity](0-2-2_ha_topologies_and_konnectivity.md)**
- 🥾 **[Module 0-10: Bootstrapping, Upgrades & etcd](0-10_maintenance_upgrades_and_etcd.md)**
- 📊 **[Module 0-14: Administration & Observability](0-14_cluster_administration_and_observability.md)**

## 🔌 Domain 2: Core API Engine & Extensibility
- 🛠️ **[Module 0-1: API Mechanics & kubectl CLI](0-1_kube_api_and_kubectl.md)**
- 🔀 **[Module 0-12: API Management & Pod Immutability](0-12_kubernetes_api_management_and_pod_immutability.md)**
- 🔌 **[Module 0-15: API Extensions & Operators](0-15_kubernetes_api_extension_and_operators.md)**
- ⛵ **[Module 12-3: Helm Package Management](12-3_helm_package_management.md)**

## ⚙️ Domain 3: Node Mechanics & Container Runtimes
- ⚙️ **[Module 0-3: Node Mechanics & Resource Limits](0-3_node_mechanics_and_resource_limits.md)**
- 🧪 **[Module 0-5: Containers, Runtimes & Lifecycles](0-5_containers_runtimes_and_lifecycle.md)**

## 📦 Domain 4: Workloads, Controllers & Scheduling
- 📦 **[Module 0-6-1: Pod Mechanics & Probes](0-6-1_pod_lifecycle_probes_and_containers.md)**
- 🔄 **[Module 0-6-2: Deployments & Rollouts](0-6-2_deployments_replicasets_and_rollbacks.md)**
- ⚙️ **[Module 0-6-3: StatefulSets, Jobs & CronJobs](0-6-3_statefulsets_daemonsets_jobs_cronjobs.md)**
- ⛵ **[Module 0-6-4: Declarative Management & Helm](0-6-4_declarative_management_helm_kustomize.md)**
- 🏥 **[Module 0-4: Workload Lifecycle & Self-Healing](0-4_workload_lifecycle_and_healing.md)**
- 🎯 **[Module 0-13-1: Scheduling Predicates & Placement](0-13-1_pod_scheduling_predicates_and_scoring.md)**
- 📌 **[Module 0-13-2: Advanced Scheduling & Eviction Control](0-13-2_advanced_scheduling_and_eviction_control.md)**
- 📜 **[Module 0-13-3: Cluster Logging & Observability](0-13-3_cluster_logging_events_and_monitoring.md)**
- 🔌 **[Module 0-13-4: Application Lifecycle & Multi-Container Patterns](0-13-4_pod_lifecycle_configurations_and_patterns.md)**

## 💾 Domain 5: Data Storage & Networking
- 📦 **[Module 0-8-1: Storage Primitives & CSI](0-8-1_storage_primitives_emptydir_hostpath.md)**
- 💾 **[Module 0-8-2: PersistentVolumes & StorageClasses](0-8-2_pv_pvc_storageclasses_and_csi.md)**
- 🕸️ **[Module 0-9-1: CNI & Service Proxying (iptables/IPVS)](0-9-1_pod_and_service_networking_iptables_ipvs.md)**
- 🔍 **[Module 0-9-2: CoreDNS Architecture & Resolution](0-9-2_coredns_resolution_and_custom_configs.md)**
- 🚪 **[Module 0-9-3: Ingress Controllers & Gateway API](0-9-3_ingress_controllers_and_gateway_api.md)**

## 🛡️ Domain 6: Cluster Security & Policy Governance
- 🎯 **[Module 0-7-0: Attack Surface, 4Cs & Threat Modeling](0-7-0_attack_surface_and_threat_modeling.md)**
- 🔑 **[Module 0-7-1: RBAC & ServiceAccounts](0-7-1_rbac_service_accounts_and_certificates.md)**
- 🛡️ **[Module 0-7-2: Pod Security Standards & Admission](0-7-2_pod_security_standards_and_admission.md)**
- 🧱 **[Module 0-7-3: NetworkPolicies & Traffic Segregation](0-7-3_network_policies_and_traffic_segregation.md)**
- 🗝️ **[Module 0-7-4: Secret Management & Encryption at Rest](0-7-4_secret_encryption_at_rest.md)**
- 📦 **[Module 0-7-5: Supply Chain Security & ImagePolicyWebhook](0-7-5_supply_chain_security_and_imagepolicywebhook.md)**
- 🔍 **[Module 0-7-6: Runtime Security (Falco) & API Auditing](0-7-6_runtime_security_falco_and_audit_logging.md)**
- 🛡️ **[Module 0-7-7: Host Operating System & Node Hardening](0-7-7_host_operating_system_and_node_hardening.md)** *(Host OS SUID/SGID auditing, sudoers & SSH hardening, service masking, socket queuing, and CIS kube-bench)*
- 🛡️ **[Module 0-7-8: Cluster Hardening, CIS Benchmarks, Platform Security & Upgrades](0-7-8_cluster_hardening_cis_benchmarks_and_upgrades.md)** *(The 4Cs, kube-bench auditing, binary checksum verification, safe upgrades, and IMDSv2 metadata protection)*
- 🔒 **[Module 0-7-9: Workload Kernel Isolation, Seccomp, AppArmor & Linux Capabilities](0-7-9_workload_kernel_isolation_seccomp_apparmor_and_capabilities.md)** *(AppArmor MAC, Linux Capabilities decomposition, syscall strace profiling, Seccomp BPF, and gVisor/Kata sandboxing)*
- 🧬 **[Module 0-16: Admission Controllers & Webhooks](0-16_admission_controllers.md)**

## 🕵️ Domain 7: Troubleshooting & Diagnostics
- 🕵️ **[Module 0-11: Cluster Troubleshooting & Diagnostics](0-11_troubleshooting_and_diagnostics.md)**
