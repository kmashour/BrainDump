...
1. **Architectural Cohesion (Anti-Fragmentation):** In Mumshad's course, a single topic like a "Pod" is scattered across several weeks (Core Concepts for basic specs, Scheduling for affinity and resource limits, Application Lifecycle for command args and sidecars, Security for securityContexts). The reference notes centralize these so that when you look at [[0-6-1_pod_lifecycle_probes_and_containers|0-6-1_pod_lifecycle_probes_and_containers.md]], you get the complete picture of how all workloads function.
...
| :-------------------------- | :--------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. Core Concepts**        | Cluster Control Plane & Pods                         | [[0-2-1_control_plane_and_core_daemons \| 0-2-1_control_plane_and_core_daemons.md]]<br>[[0-6-1_pod_lifecycle_probes_and_containers \| 0-6-1_pod_lifecycle_probes_and_containers.md]]       | Control Plane components, API Server flow<br>Pod anatomy, CLI speed hacks, and namespaces                                                            |
| **2. Scheduling**           | Placement, Affinity, resource limits, DaemonSets     | [[0-3_node_mechanics_and_resource_limits \| 0-3_node_mechanics_and_resource_limits.md]]<br>[[0-13-1_pod_scheduling_predicates_and_scoring \| 0-13-1_pod_scheduling_predicates_and_scoring.md]]               | CPU/Mem requests & limits, cgroup v1/v2 mapping<br>Taints/Tolerations, Affinity rules, Custom Schedulers                                             |
| **3. Logging & Monitoring** | Metrics Server & logging sidecars                    | [[0-13-1_pod_scheduling_predicates_and_scoring \| 0-13-1_pod_scheduling_predicates_and_scoring.md]]<br>[[0-14_cluster_administration_and_observability \| 0-14_cluster_administration_and_observability.md]] | Logging architectures, tailing stdout/stderr<br>Metrics Server API, CPU/Memory profiling                                                             |
| **4. App Lifecycle**        | Rolling updates, ConfigMaps, Secrets, initContainers | [[0-6-1_pod_lifecycle_probes_and_containers \| 0-6-1_pod_lifecycle_probes_and_containers.md]]<br>[[0-7-4_secret_encryption_at_rest \| 0-7-4_secret_encryption_at_rest.md]]                   | Deployment rolling update parameters (`maxSurge`/`maxUnavailable`) & rollbacks<br>ConfigMap/Secret injection (envs, files)                           |
...
| **6. Security**             | TLS certificates, RBAC, NetworkPolicies, Webhooks    | [[0-7-1_rbac_service_accounts_and_certificates \| 0-7-1_rbac_service_accounts_and_certificates.md]]<br>[[0-7-3_network_policies_and_traffic_segregation \| 0-7-3_network_policies_and_traffic_segregation.md]]<br>[[0-16_admission_controllers \| 0-16_admission_controllers.md]] | TLS handshake, CA creation, Kubeconfig configs<br>RBAC binding, ServiceAccounts, NetworkPolicy rules<br>Admission webhooks, ImagePolicyWebhook setup |
| **7. Storage**              | PV, PVC, StorageClasses, CSI drivers                 | [[0-8-1_storage_primitives_emptydir_hostpath \| 0-8-1_storage_primitives_emptydir_hostpath.md]]<br>[[0-8-2_pv_pvc_storageclasses_and_csi \| 0-8-2_pv_pvc_storageclasses_and_csi.md]]                                    | CSI architecture, PV bindings, Mount volumes, StorageClasses                                                                                          |
| **8. Networking**           | Services, CoreDNS, CNI plugins, Ingress              | [[0-9-1_pod_and_service_networking_iptables_ipvs \| 0-9-1_pod_and_service_networking_iptables_ipvs.md]]                                                                                                                        | Service iptables/IPVS routing, CoreDNS resolution<br>CNI packet headers, Ingress controller setups                                                   |
...
*   **Step 1.3:** Study **Control Plane components**. Read [[0-2-1_control_plane_and_core_daemons|0-2-1_control_plane_and_core_daemons.md]] to dissect the API server loop, scheduler pipelines, and kubelet state syncs.
*   **Step 1.4:** Master **Workloads (Pods/Deployments)**. Study [[0-6-1_pod_lifecycle_probes_and_containers|0-6-1_pod_lifecycle_probes_and_containers.md]] (Sections 1-4) for workload YAML structure, rolling updates, and rollback strategies.
...
*   **Step 2.2:** Master **Scheduling Logic**. Read [[0-13-1_pod_scheduling_predicates_and_scoring|0-13-1_pod_scheduling_predicates_and_scoring.md]] to study Taints, Tolerations, Node Affinity, PriorityClasses, and custom scheduler leases.
*   **Step 2.3:** Inject **State & Credentials**. Study [[0-7-4_secret_encryption_at_rest|0-7-4_secret_encryption_at_rest.md]] for ConfigMaps and Secrets, and how files/env-vars are projected into container runtimes.
...
*   **Step 4.1:** Study **Persistent Storage**. Read [[0-8-1_storage_primitives_emptydir_hostpath|0-8-1_storage_primitives_emptydir_hostpath.md]] and [[0-8-2_pv_pvc_storageclasses_and_csi|0-8-2_pv_pvc_storageclasses_and_csi.md]] to learn how PersistentVolumes bind to claims, the role of StorageClasses, and CSI controller plugin pipelines.
*   **Step 4.2:** Study **CNI & Services**. Read [[0-9-1_pod_and_service_networking_iptables_ipvs|0-9-1_pod_and_service_networking_iptables_ipvs.md]]. Focus on:
...
*   **Step 5.1:** Study **TLS, RBAC, and Policies**. Read [[0-7-1_rbac_service_accounts_and_certificates|0-7-1_rbac_service_accounts_and_certificates.md]] to understand X.509 cert chains, Role/ClusterRole bindings, and [[0-7-3_network_policies_and_traffic_segregation|0-7-3_network_policies_and_traffic_segregation.md]] for NetworkPolicy ingress/egress CIDR blocks.
*   **Step 5.1a:** Learn **Admission Controllers and Webhooks**. Read [[0-16_admission_controllers|0-16_admission_controllers.md]] to master admission phases, webhook configurations, dynamic Python Flask webhooks, and `ImagePolicyWebhook` static-pod config integrations.
