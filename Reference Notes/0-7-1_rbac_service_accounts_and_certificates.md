...
## 8. Kubelet Security, NodeRestriction & Certificate Audits

### 8.1 Kubelet Authentication & Authorization Hardening
By default, an unhardened Kubelet allows unauthenticated access to its HTTP endpoints (port 10250), which enables attackers to execute arbitrary commands inside containers via `/run/` or retrieve pod logs.

#### Hardening `/var/lib/kubelet/config.yaml`:
```yaml
...
# 3. Disable insecure legacy read-only port (Port 10255)
readOnlyPort: 0
...
After modifying the configuration, restart the kubelet daemon:
```bash
...
### 8.2 NodeRestriction Admission Controller
The `NodeRestriction` admission plugin limits the Node and Pod objects that a Kubelet can modify:
* A Kubelet can only modify its own `Node` API object.
* A Kubelet can only modify `Pod` objects bound to its own node.
* A Kubelet cannot modify labels with prefixes `node-restriction.kubernetes.io/`.
* A Kubelet cannot add labels to its node that begin with `node-role.kubernetes.io/`.

Enable `NodeRestriction` on the kube-apiserver:
```yaml
# In /etc/kubernetes/manifests/kube-apiserver.yaml
- --enable-admission-plugins=NodeRestriction
```

### 8.3 X.509 Certificate Expiration Auditing
Kubernetes components authenticate using TLS client certificates signed by the cluster CA (`/etc/kubernetes/pki/ca.crt`). Certificates generated by `kubeadm` expire after **1 year**.

```bash
# 1. Check all kubeadm-managed certificate expiration dates
kubeadm certs check-expiration

# 2. Inspect a specific certificate file with OpenSSL
openssl x509 -in /etc/kubernetes/pki/apiserver.crt -text -noout | grep -A 2 "Validity"
# Check Subject Alternative Names (SANs)
openssl x509 -in /etc/kubernetes/pki/apiserver.crt -text -noout | grep -A 1 "Subject Alternative Name"

# 3. Renew all certificates manually before expiration
kubeadm certs renew all

# 4. Restart static pods after renewal so new certificates take effect
